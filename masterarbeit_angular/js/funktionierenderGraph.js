...
//               });
            ...
            $http.post('databaseFiles/getRelation.php', $routeParams.techid)
                .then(function (info) {
                    ...
                    for (var i = 0; i < $scope.relationen.data.length; i++) {
                        newDataset.push(
                            {source:$routeParams.param, target: ($scope.relationen.data[i].tech_name).toString(), type: "licensing", cat:"framework"}
                        );
                    }
                    getOtherNodes(newDataset, $scope.relationen.data);
    ...
    function getOtherNodes(newDataset, relationen) {

        console.log(relationen);
        if (relationen.length != 0) {
            //wie viele elemente hat technologie[1]
            console.log(relationen);
            for (var j = 0; j < relationen.length; j++) {
                $http.post('databaseFiles/getRelation.php', relationen[j].tech_id)
                    .then(function (info) {
                        ...
                        for (var k = 0; k < info.data.length; k++) {
                            ...
                            newDataset.push(
                                {source:"testTech5", target: (info.data[k].tech_name).toString(), type: "suit"}
                            );
                            console.log(newDataset);
                            update(newDataset);
                        }
        ...
        return new Promise(function (resolve, reject) {
            $http.post('databaseFiles/getRelation.php', $routeParams.techid)
                .then(function (info) {
                    ...
                    {name: $routeParams.param, id: $routeParams.techid, cat: "framework"}
                    );
                    //TEchnologie ansich
                    //Abhängigkeiten 1, Stufe zu node hinzufügen
                    for (var i = 0; i < $scope.relationen.data.length; i++) {
                        ...
                        links.push(
                            {
                                source: nodes[$routeParams.param],
                                target: nodes[($scope.relationen.data[i].tech_name).toString()],
                        ...
                        if(info.data.length!=0){
                            ...
                            for (var k = 0; k < info.data.length; k++) {
                                ...
                                for(var i=0; i< nodes.length; i++){
                                    if(nodes[i].id==nodes[info.data[k].r_ab_id]){
                                        source=nodes[i];
                                    }
                                }
